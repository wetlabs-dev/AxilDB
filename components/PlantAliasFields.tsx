...
const control = 'rounded-md border border-stone-300 bg-[#fffdf7] px-3 py-2 text-sm font-normal shadow-inner shadow-stone-200/30 outline-none transition focus:border-[#2f6b45] focus:ring-2 focus:ring-[#8fa58f]/30'
...
export function PlantAliasFields({ aliases = [] }: { aliases?: Alias[] }) {
  const rows = [
    ...aliases,
    ...Array.from({ length: Math.max(3, 5 - aliases.length) }, () => ({} as Alias)),
  ]
    ...
    <div className="md:col-span-2">
      ...
      <div className="grid gap-3">
        {rows.map((alias, index) => (
          <div key={alias.id || index} className="grid gap-2 rounded-lg border border-stone-200 bg-[#fffdf7]/70 p-3 md:grid-cols-5">
            <label className="grid gap-1.5 text-sm font-medium text-stone-800 md:col-span-2">
              Name
            ...
            <label className="grid gap-1.5 text-sm font-medium text-stone-800 md:col-span-5">
              Notes
