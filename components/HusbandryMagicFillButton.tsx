    ...
    <div className={cn('grid min-w-0 justify-items-end gap-2', className)}>
      ...
      {substrateRecommendation && (
        <div className="w-full max-w-xl rounded-lg border border-[#d6dfc9] bg-[#f7f4e8] p-3 text-left text-sm text-stone-700 shadow-sm">
          ...
          {substrateRecommendation.newRecipeSuggestion && <div className="mt-3 rounded-md border border-stone-200 bg-white/65 p-2"><p className="font-semibold text-stone-900">Suggested draft: {substrateRecommendation.newRecipeSuggestion.name}</p>{substrateRecommendation.newRecipeSuggestion.reason && <p className="mt-1 text-xs">{substrateRecommendation.newRecipeSuggestion.reason}</p>}<SubstrateCompositionBar className="mt-2" items={substrateRecommendation.newRecipeSuggestion.components.map((item: any) => ({ id: item.componentId, percentByVolume: item.percentByVolume, component: item.component }))} mode="compact" /><div className="mt-2"><input type="hidden" name="collectionSlug" value={collectionSlug} /><input type="hidden" name="name" value={substrateRecommendation.newRecipeSuggestion.name} /><input type="hidden" name="description" value={substrateRecommendation.newRecipeSuggestion.reason || ''} /><input type="hidden" name="status" value="DRAFT" />{substrateRecommendation.newRecipeSuggestion.components.map((item: any) => <span key={item.componentId}><input type="hidden" name="substrateComponentId" value={item.componentId} /><input type="hidden" name="percentByVolume" value={item.percentByVolume} /><input type="hidden" name="componentNotes" value="" /></span>)}<button type="submit" formAction={createSubstrateRecipe} className="rounded-md border border-[#2f6b45] px-3 py-1.5 text-xs font-semibold text-[#2f6b45]">Create recipe draft</button></div></div>}
        </div>
      )}
