...
export function AIDescriptionField({
  ...
  async function magicFillDefinition() {
    ...
    try {
      ...
      setStatus('Magic fill added. Review before saving.')
    } catch (error) {
    ...
    <label className={cn('grid min-w-0 gap-1 text-sm font-medium text-stone-800', wrapperClassName)}>
      <span className="flex min-w-0 flex-wrap items-center justify-between gap-2">
        ...
        <button
          ...
          disabled={loading || magicLoading}
          className="inline-flex items-center gap-1.5 rounded-md border border-[#c4a86a]/60 bg-[#fff5d6] px-2 py-1 text-xs font-semibold text-[#6f541f] shadow-sm transition hover:bg-[#f7e6ae] disabled:cursor-wait disabled:opacity-70"
        >
          <Sparkles className="h-3.5 w-3.5" />
          {magicLoading ? 'Filling...' : 'Magic fill'}
        </button>
      </span>
      <textarea
        ref={textareaRef}
