...
Treatments are managed from **Treatments** in the Admin sidebar by gardeners and managers. A treatment definition stores reusable targets, instructions, reference dose, products, applicable condition types, repeat/follow-up defaults, PPE, quarantine, ventilation, indoor-use, bloom, heat, light, re-entry, contraindication, and plant-tag caution metadata. Products store physical inventory and label/SDS references separately. Treatment plans expand into concrete dated application and assessment steps, which appear in the Care Queue and link back to the plan. Loggers and above record actual application values and follow-up outcomes; plan completion requires all required steps plus a final outcome and effectiveness rating. One-off applications are also supported. Historical applications snapshot instructions and safety details so later library edits do not rewrite the record. Collection outcome summaries are descriptive local history, not scientific evidence. AxilDB never replaces the current product label, local regulations, or professional advice; verify dose, compatibility, PPE, ventilation, environment, and re-entry requirements before every application.
