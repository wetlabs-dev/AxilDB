...
Acquisition labels belong to the physical Plant Instance, because two specimens of the same definition may arrive under different nursery or seller labels. Acquisition and bulk-acquisition forms copy the supplied label only to the specimens they create. A Plant Definition may instead use a **Provisional / working taxon** when its identity is unresolved. That value takes precedence as the displayed name and adds a **Needs identification review** warning, while AxilDB retains a working genus/species placement for plant ID generation. Clearing the provisional value requires an identified genus and species, and provisional definitions cannot be promoted to site-level Validated Definitions. After correcting the identity, managers can use the existing plant ID regeneration action where the expected ID has changed.
