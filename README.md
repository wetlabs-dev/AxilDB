...
Fertilizers are managed from **Fertilizers** in the Admin sidebar by gardeners and managers. Products store brand/type/NPK/dilution notes; recipes combine products with application method, strength, dose, water volume, cadence, seasonal notes, and safety notes. Plant definition husbandry can assign an active recipe and cadence, and specimen pages can override or disable fertilizing locally. Completing a fertilizing task records a `FERTILIZED` care event with recipe and dose metadata. Magic Fill Husbandry may recommend an existing collection recipe or suggest a new recipe draft, but it never assigns or creates fertilizer records until the user explicitly chooses the recommendation and saves.
