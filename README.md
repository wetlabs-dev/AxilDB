...
- Collection-scoped fertilizer products and recipes, with structured NPK/product metadata, husbandry recipe assignment, fertilizing cadence, specimen overrides, and fertilizing tasks in the Care Queue.
- Collection-scoped treatment management with reusable instructions and products, condition applicability, safety constraints, dated treatment plans, actual application records, outcomes, Care Queue steps, specimen timelines, and local effectiveness summaries.
...
- AI Magic Fill for complete husbandry guide drafts, stored as editable draft content with model/review metadata, including cautious fertilizer recipe recommendations that require user confirmation.
...
- Smart Care Queue that combines husbandry cadence, watering/fertilizing history, propagation stage, open health conditions, bloom follow-ups, pest checks, and manual reminders into a prioritized collection worklist.
...
- Plant instances with generated plant IDs, specimen-specific acquisition labels, lifecycle-aware dates, source/distributor metadata, hierarchical locations, archive status, notes, photos, Seed/Corm/Tissue Culture transitional lifecycle states, explicit Mark established transitions, and irreversible **Pot Together** lifecycle merges. A merge preserves the survivor's ID/QR code, keeps constituent IDs searchable as read-only historical records, aggregates their timelines with origin labels, and carries unresolved operational records forward.
...
- Automatic plant ID generation based on plant definition, lifecycle date context, context code, and sequence number, including SD for Seed, CO for Corm, and TC for Tissue Culture.
- Propagation events with parent/child links, method, resulting child instance type, date, success status, and generated child plant IDs.
...
- Collection-gardener edit/delete tools, taxonomic authorities page, and audit log.
...
**Taxonomic Authorities** replaces the former Governing Bodies lookup. Each authority can represent an ICRA, scientific society, government agency, botanical institution, committee, registry, or another future authority type. Records store official website, registration, cultivar-search, membership, and canonical external-record links without copying registrar details that may become stale. Publications and structured scope rules are maintained inside expandable authority cards at `/c/[collectionSlug]/taxonomic-authorities`.

Scope rules use ranks rather than free text. The schema supports kingdom through species, while genus, tribe, family, and order are the practical initial ranks. When a plant definition is created or its taxonomy changes, AxilDB records every matching authority and selects the most specific rule; a species match outranks genus, genus outranks family, and family outranks order. Rule priority breaks ties at the same rank. Managers can re-evaluate a collection at any time. A manual override is preserved through later rematches, while **Continue without authority** dismisses a match without blocking the plant definition.

Magic Fill receives only compact authority names and scope rules. It may display the deterministic match implied by the drafted accepted genus, but it cannot create or assign an authority by itself. Saving the reviewed plant definition runs the normal matcher. Authority diagnostics report unmatched definitions, ambiguous matches, overrides, authorities without scope, and malformed resource URLs. The read-only `/api/taxonomic-authorities?collection=[slug]` endpoint respects collection visibility and membership and exposes public-safe metadata, scope, publications, and an optional plant definition's match reason.

The import helper in `lib/taxonomic-authority-import.ts` is provider-neutral. Future ISHS ICRA, CSV, or JSON importers can update official metadata, scopes, and publications while keeping local notes and manual plant overrides. Registration workflow fields are reserved in the plant definition schema but remain hidden until those workflows are implemented.
...
- **Gardener** can edit, delete, archive/restore, manage taxonomic authorities, select cover/type photos, review audit logs, and run collection tools.
...
Care Schedule Sync is available from the Care Queue, Plant Instances, and location detail pages for loggers and above. It aligns selected active plants and care types, including fertilizing, to a reviewed next-due date without creating care events, deleting history, or changing cadence by default. Users can explicitly choose **Sync cadence as well** to set the same cadence override, such as a 5-day or 7-day watering interval, across the selected plants and care types. Collection managers can define collection-level Quiet Days in Collection Settings, including one-time dates, weekly recurring days, and date ranges. Care Queue and Care Schedule Sync shift affected due dates off Quiet Days using per-care-type rules and show the reason when a date was adjusted.

Fertilizers are managed from **Fertilizers** in the Admin sidebar by gardeners and managers. Products store brand/type, structured guaranteed-analysis percentages for NPK, secondary nutrients, and micronutrients, manufacturer label feed-rate fields, usage notes, source URL/name, confidence, and review metadata. Legacy freeform micronutrient text is preserved as guaranteed-analysis notes. Recipes combine products with the collection’s actual application method, strength, dose, water volume, cadence, seasonal notes, and safety notes; product feed rate is manufacturer label guidance, while recipe dilution is the collection’s chosen mix. Plant definition husbandry can assign an active recipe and cadence, and specimen pages can override or disable fertilizing locally. Completing a fertilizing task records a `FERTILIZED` care event with recipe and dose metadata. Magic Fill Product can draft product-label data from public/manufacturer sources for review, and Magic Fill Husbandry may recommend an existing collection recipe or suggest a new recipe draft. Neither workflow saves, assigns, or creates fertilizer records until the user explicitly applies the draft and saves.

Treatments are managed from **Treatments** in the Admin sidebar by gardeners and managers. Definitions store reusable instructions, structured product recipes, condition suitability, intervals, environmental limits, pet/aquatic safety, PPE, quarantine, re-entry, incompatibility, disposal, sources, and plant-tag cautions. Plans may use generated or manager-authored dated steps, appear in the Care Queue, accept assessment photos, and start or release quarantine through explicit steps. Loggers and above record actual values, immediate responses, follow-up dates, outcomes, and corrections; closing a plan includes an explicit linked-condition review. One-off and filtered batch/location applications support per-plant overrides, idempotent recording, warning snapshots, and matching to compatible open conditions and pending plan steps. Reports and CSV export summarize usage and outcomes with sample-size caveats. Historical applications snapshot instructions and safety details so later library edits do not rewrite the record. Treatment data remains private to authenticated collection members even when a collection has public pages. AxilDB never replaces the current product label, local regulations, or professional advice; verify dose, compatibility, PPE, ventilation, environment, and re-entry requirements before every application.

The Acquisition Pipeline is available from **Wishlist** and **Acquisitions** in the Collection sidebar. It is intentionally definition-centered rather than a separate wishlist table: a Plant Definition can be Researching, Wishlist, Actively Seeking, On Hold, Fulfilled, or No Longer Interested independent of whether the collection already owns specimens. Wishlist offers filterable cards and a compact Shopping view. Members can select several definitions and review one idempotent acquisition batch before AxilDB creates the permanent acquisition records and optional Plant Instances. CSV and branded PDF exports are available from the wishlist.

Acquisition labels belong to the physical Plant Instance, because two specimens of the same definition may arrive under different nursery or seller labels. Acquisition and bulk-acquisition forms copy the supplied label only to the specimens they create. Plant Definitions support specified species (`Monstera deliciosa`), intentionally omitted epithets (`Begonia 'Looking Glass'`), and genuinely undetermined species (`Begonia sp.`) as distinct states. Blank species values stay blank in exports and APIs; new plant IDs use the genus and cultivar segments without placeholder or doubled separators. Existing names and IDs are not rewritten. A Plant Definition may instead use a **Provisional / working taxon** when its identity is unresolved. That value takes precedence as the displayed name and adds a **Needs identification review** warning, while AxilDB retains a working placement for plant ID generation. Clearing the provisional value requires an accepted identity, which may be genus and cultivar when horticultural usage omits the species. Provisional definitions cannot be promoted to site-level Validated Definitions. After correcting the identity, managers can use the existing plant ID regeneration action where the expected ID has changed.

New Plant Instance creation uses a cascading Plant Definition picker instead of one large definition dropdown. Genus, Species, Hybrid, and Cultivar progressively narrow existing local and usable Validated Definitions; the Species step explicitly separates **No species / genus-level cultivar** from **sp. - species unknown**. Seed, Corm, and Tissue Culture are current lifecycle states for one physical accession, not lineage models or batch records. Known seed parents, corm origins, and tissue-culture propagation remain represented through Propagation records. The specimen page offers **Mark established** for those transitional states, records the establishment date and notes, emits lifecycle events, and changes the current type to Mother or Propagation without erasing the original timeline.

Collection Settings controls wishlist visibility and individual public fields. Public fields are off by default; exact internal locations, maximum-price ceilings, research journals, private observation notes, distributor experience data, and non-public observations are never included in the public projection. An observation must be explicitly marked public before it can contribute to a public date or price summary. Public visitors receive a light-mode read-only view and cannot run acquisition actions.

Collection Exhibits can curate wishlist definitions in a separate **Planned Acquisitions** section with independent order, featured state, and captions. These entries remain Plant Definitions and never create fake specimens. ID My Plant history can add a local match to the wishlist or open the existing review form to create an unmatched definition first. Plant Definition Magic Fill can return a sourced acquisition-planning draft and location-fit suggestion; applying selected draft fields is explicit and never changes wishlist status or priority.

**Provenance** now uses three user-facing concepts. A Source is a breeder, grower, propagator, importer, collector, or other upstream origin in an ordered source chain. A Seller is the person or organization that actually transferred the plant. A Sales Channel is how that seller transacted: a website, Palmstreet or Etsy profile, retail store, nursery, show booth, social account, auction, or custom channel. Managers maintain reusable channel types, merge duplicate sources/sellers/channels, archive referenced records, permanently delete only unused records, and review legacy outlet conversions from the Provenance cleanup panel. Ratings, experience notes, and physical addresses remain collection-private. Public source, seller, and sales-channel visibility is independently configurable and defaults off.

Legacy `source`, `distributor`, `vendor`, distributor, and outlet data remains in place for audit compatibility. The sales-channel migration automatically adds seller/channel links to legacy outlet-linked acquisitions, observations, and batches while preserving their original IDs and references. Existing records may also be converted explicitly from the Provenance cleanup panel. New acquisition forms ask only who the plant came from, how it was acquired, and its source chain; the legacy platform fields are inferred internally. Finish deployment verification with `npm run provenance:check`. See [docs/PROVENANCE_MODEL.md](docs/PROVENANCE_MODEL.md).

Plant Instance pages show a compact read-only **Acquisition & provenance** summary. Users with record-entry access can open **Manage acquisition** to edit the acquisition date and label, seller, sales channel, source chain, price, size, and notes. If an older specimen has no canonical acquisition record, saving this form creates and links one silently. Ordinary specimen editing no longer changes acquisition history. The Acquisition page begins with collection purchase history; wishlist and research sections remain planning tools and do not replace canonical acquisition records.
...
- `./backups:/app/backups`: sitewide backup output generated by the backup worker and mounted read-only into the app container for the backup browser.
...
- `TaxonomicAuthority`: registration or standards organizations.

Validated Plant Definitions are site-level `PlantDefinition` records with `collectionId = null` and `isValidated = true`. They are not owned by the collection that nominated them, so deleting or archiving a collection does not delete approved validated definitions or break plant instances linked to them. Collection managers can nominate a local definition for validation, server admins review nominations under Server Management, and approval creates a site-level validated definition with copied taxonomy, aliases, husbandry, type-image metadata, and taxonomic authority metadata. Managers can dispute validated definitions or create a local copy for selected specimens when they need to detach from future validated updates. Specimen-level husbandry overrides remain available, so collections do not need to detach solely for local care differences.

Most domain records carry `collectionId`, including local plant definitions, aliases, plant instances, locations, location types, plant location moves, propagations, blooms, notes, photos, reminders, follows, sunshine, taxonomic authorities, and audit logs. Suggestions/autocomplete, search, gallery, lineage graphs, labels, dashboard activity, follow counts, and sunshine counts are scoped per collection. Validated plant definitions, their aliases, husbandry guides, and type-image metadata intentionally remain site-level.
...
Substrate composition is also rendered as a shared color-and-pattern bar throughout recipe, plant, search, Care Queue, and Magic Fill views. Component colors, hatch patterns, short labels, and visual families can be edited without changing recipe history. The live recipe editor exposes unallocated or over-allocated percentages, and the Recipe Library can compare two to four versions side by side.
...
Open **Workflows** from the collection sidebar to manage repeatable greenhouse operating procedures. AxilDB provides read-only starter templates for new-arrival quarantine, weekly greenhouse rounds, pest response, seasonal moves, bloom review, and propagation checks. Collection managers can create custom templates, copy starter templates before editing, drag steps into order with keyboard move fallbacks, and archive or delete unused custom templates that should no longer be used.

Workflow runs are manual in v1 and can be scoped to the whole collection, a location with child locations included by default, or selected plant instances. Runs can be started from Workflows, the Dashboard workflow launchpad, Care Queue, the plant list selection bar, specimen pages, location pages, and quarantine context. Gardeners can start and cancel/complete runs; loggers can complete steps when the step creates records they are already allowed to create. Viewers and public visitors cannot run workflows. A whole run can be assigned to a user, while per-step assignment is intentionally deferred.

Typed workflow steps can create real AxilDB records: care events, notes, reminders, photo records, plant location moves, quarantine records, quarantine releases, and condition records. Photo steps upload through the same photo processing pipeline used elsewhere and complete after linking the image to the selected plant, location, or collection. Workflow-created care events clear matching due care queue items/reminders where appropriate. Workflow-created care events and move/quarantine records appear in Plant Health Timeline, and timeline entries also show workflow start/completion events for involved plants.
...
Managers can add a structured environment profile to any location: day/night temperature ranges, humidity, qualitative and measured light, photoperiod, airflow, stability, equipment/context, measurement source, date, confidence, and notes. Blank child fields inherit the nearest nonblank ancestor value without copying it into the child record. Location detail pages distinguish local and inherited values, identify their source, show profile completeness and stale-measurement guidance, and summarize compatibility for directly assigned plants.
...
Plant definition forms include optional OpenAI buttons. **AI draft** uses the genus, optional species, and cultivar fields to draft a description under 40 words. **Magic fill** asks OpenAI for a structured plant definition draft: accepted genus/species, authority, cultivar registration number, taxonomic authority, taxonomy/reference URLs, description, and aliases. It leaves Species blank when the accepted horticultural name intentionally omits an epithet and uses `sp.` only for a genuinely undetermined species. **ID My Plant** suggests a cautious identification draft from a user description, known common/trade names, and an optional plant image; it saves each result to ID My Plant history, marks applied suggestions as **AI Determined**, and never saves or overwrites the definition until the user clicks Apply and saves the form. **Magic Fill husbandry** asks for one structured draft covering the entire care guide and can recommend fertilizer recipe usage. **Magic Fill product** on Fertilizers searches public product-label/manufacturer data for guaranteed analysis and label feed rate, then fills the form only after the user applies the draft. Across these form-filling tools, empty forms proceed directly; when managed fields already contain information, AxilDB asks whether to **Fill Missing Only**, **Replace All Fields**, or cancel before requesting/applying the draft. Fill Missing preserves meaningful strings, numbers, booleans, arrays, nested values, and selected relationships. Magic Fill never saves automatically, including husbandry drafts, which require an explicit review and save. **Green Thumb assist** lets a logger ask one care question per plant specimen per day and stores the answer as a highlighted care note. **Collection Briefing** can be enabled by a collection manager after server-level AI access is enabled; it creates one cached dashboard briefing per collection local day when an active authenticated member first opens the dashboard, written as a short conversational note rather than a comprehensive checklist. The API key is never sent to the browser, and generated definition/husbandry/fertilizer data is never saved automatically; review it before saving.
...
- Server health incidents for memory, disk, backup worker, reminder worker, metrics worker, image moderation worker, SMTP delivery failures, delivery backlog, AI, and moderation failures. Memory incidents open after three consecutive samples above 75% warning or 90% critical; disk opens as soon as usage exceeds 80% warning or 90% critical; metric incidents resolve after three clear samples. Worker incidents resolve when the next worker run succeeds. Server admins can search/filter history and create manual incidents for operational events such as migrations, outages, reboots, and storage work.
- Server health alert emails and push alerts for verified `SERVER_ADMIN` users when the metrics worker sees degraded disk or memory health. These alerts default to a 6-hour per-admin cooldown and attach delivered or failed notification records to open incidents.
