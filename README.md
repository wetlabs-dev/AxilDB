...
- Acquisition Pipeline for pre-accession research and wishlist intent on plant definitions, with structured upstream sources, reusable sellers and marketplace storefronts, distributors and distributor-operated outlets, observations, price analytics, and acquisition records that can create one or more Plant Instances.
...
**Provenance** separates the upstream source, actual seller, seller storefront, transaction channel, and channel-operated outlet. A Source is a breeder, grower, propagator, importer, collector, or other origin in an ordered source chain. A Seller is the person or organization that actually transferred the plant; reusable Seller Storefronts represent that seller on Palmstreet, Etsy, a direct site, or another channel. A Distributor is the marketplace, retailer, nursery, auction platform, or sale channel. Distributor Outlets are branches, direct online stores, or event booths operated by that distributor, not independent marketplace accounts. Ratings and experience notes remain collection-private. Public source, seller, storefront, distributor, and outlet visibility is separately configurable and defaults off.

Legacy `source`, `distributor`, and `vendor` text remains in place during rollout. Run `npm run provenance:migrate -- --dry-run` before the original structured migration. After deploying Seller/Storefront support, run `npm run provenance:marketplace-migrate -- --dry-run`, review the classification, then run it without `--dry-run`. Obvious handle-like marketplace outlets become reusable seller storefronts; ambiguous records enter reconciliation without losing their original text. Finish with `npm run provenance:check`. See [docs/PROVENANCE_MODEL.md](docs/PROVENANCE_MODEL.md).
