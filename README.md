...
Care Schedule Sync is available from the Care Queue, Plant Instances, and location detail pages for loggers and above. It aligns selected active plants and care types to a reviewed next-due date without creating care events, deleting history, or changing cadence by default. Collection managers can define collection-level Quiet Days in Collection Settings, including one-time dates, weekly recurring days, and date ranges. Care Queue and Care Schedule Sync shift affected due dates off Quiet Days using per-care-type rules and show the reason when a date was adjusted.
