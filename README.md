...
**Sources & Distributors** separates upstream provenance from the direct transaction. A Source is a breeder, grower, propagator, importer, collector, or other origin in an ordered acquisition source chain. A Distributor is the seller, trader, donor, or outlet from which the collection directly received the material; reusable branch and online-outlet records belong beneath it. Ratings and experience notes are private to the collection. Public source provenance is separately configurable, while distributor identity and distributor location default to private. Plant Definitions remain botanical records and do not own commercial provenance.

Legacy `source`, `distributor`, and `vendor` text remains in place during rollout. Run `npm run provenance:migrate -- --dry-run` first; clean single-name values can then be linked idempotently with `npm run provenance:migrate`, while combined or uncertain strings enter the reconciliation queue without losing their original text. Run `npm run provenance:check` after migration. See [docs/PROVENANCE_MODEL.md](docs/PROVENANCE_MODEL.md) for collection-boundary, merge, and future shared-record design.
