...
export type CollectionContext = {
  collection: {
    ...
    wishlistIntro: string | null
    wishlistPublicSettingsJson: unknown
    showSourceProvenance: boolean
    showDistributorIdentity: boolean
    showDistributorOutlet: boolean
    showSellerIdentity: boolean
    showSellerStorefront: boolean
...
export async function backfillDefaultCollection(collectionId?: string) {
  ...
  await prisma.$transaction([
    ...
    prisma.plantCareEvent.updateMany({ where: { collectionId: null }, data: { collectionId: id } }),
    prisma.plantCondition.updateMany({ where: { collectionId: null }, data: { collectionId: id } }),
    prisma.plantCareAdjustment.updateMany({ where: { collectionId: null }, data: { collectionId: id } }),
