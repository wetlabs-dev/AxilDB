...
function publicPageHeaders(request: NextRequest) {
  const requestHeaders = requestHeadersWithPath(request)
  requestHeaders.set('x-axildb-public-page', '1')
  return requestHeaders
}
...
export function middleware(request: NextRequest) {
  ...
  if (pathname.startsWith('/exhibit/') || pathname.startsWith('/exhibit-subscription/')) {
    const requestHeaders = publicPageHeaders(request)
    requestHeaders.set(RETURN_TO_HEADER, `${pathname}${search}`)
    return NextResponse.next({
      request: {
        headers: requestHeaders,
