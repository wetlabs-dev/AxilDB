...
async function main(){
 ...
 await prisma.user.create({data:{email:'admin@axildb.com',passwordHash:hashPassword('password'),role:'ADMIN'}})
 ...
 const chimera=await prisma.plantDefinition.create({data:{genus:'Saintpaulia',species:'ionantha',cultivarName:'Demo Starfall',authority:'H.Wendl.',cultivarRegistrationNumber:'DEMO-001',governingBodyId:avsa.id,confidence:'PROBABLE',acquisitionLabel:'African violet Demo Starfall',provisionalTaxon:'Saintpaulia ionantha',description:'Demo cultivar for seeded development data.',aliases:{create:[{name:'African violet',aliasType:'COMMON_NAME',confidence:'CONFIRMED',source:'Common horticultural use'},{name:'Streptocarpus sect. Saintpaulia Demo Starfall',aliasType:'SYNONYM',confidence:'UNCERTAIN',source:'Taxonomic review placeholder'}]}}})
