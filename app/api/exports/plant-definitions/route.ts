...
const baseHeaders = [
  ...
  'taxonomicAuthority',
  'taxonomicAuthorityAbbreviation',
  'confidence',
...
export async function GET(request: Request) {
  ...
  const rows = definitions.map((definition) => {
    ...
    definition.taxonomicAuthority?.name,
    definition.taxonomicAuthority?.abbreviation,
    ...
    definition.tags.map((item) => item.plantTag.name).join('; '),
