...
import { PlantTagFields } from '@/components/PlantTagFields'
import { AddPanel, Button, Card } from '@/components/ui'
import { requireCollectionGardener } from '@/lib/collections'
import { plantTagCategories, tagCategoryLabel } from '@/lib/plant-tags'
...
export default async function PlantTagsPage({ searchParams }: { searchParams: Promise<{ q?: string; category?: string; sort?: string }> }) {
  ...
  return <div className="space-y-6">
    ...
    <AddPanel label="Create plant tag"><form action={savePlantTag} className="grid gap-3 md:grid-cols-4"><input type="hidden" name="collectionSlug" value={collection.slug} /><PlantTagFields /><Button className="justify-self-start md:col-span-4">Create tag</Button></form></AddPanel>
    ...
    <div className="grid gap-3 lg:grid-cols-2">{tags.map((tag) => <Card key={tag.id} className={!tag.active ? 'opacity-75' : ''}>
      ...
      <details className="mt-3 rounded-md border border-[color:var(--ax-border)] p-3"><summary className="cursor-pointer font-semibold">Edit</summary><form action={savePlantTag} className="mt-3 grid gap-3 md:grid-cols-4"><input type="hidden" name="collectionSlug" value={collection.slug} /><input type="hidden" name="id" value={tag.id} /><PlantTagFields tag={tag} /><Button className="justify-self-start md:col-span-4">Save tag</Button></form></details>
