...
Account settings manage your email address, password, email verification, two-factor authentication, recovery codes, measurement units, and notification preferences.
...
- Start a treatment plan from Treatment Management, Care Queue condition actions, Green Thumb, or an open condition on a specimen. Choose generated dates or review a custom step draft before starting. Active steps can be edited, added, or skipped with a reason; photo steps require an uploaded image, and quarantine steps update the specimen quarantine record.
...
- Use Batch application to filter active specimens by location and descendants, condition, severity, quarantine, or search. Shared values can be overridden per specimen. AxilDB rechecks every selection, stores warnings per application, prevents duplicate submission, and completes a matching pending plan step when one exists.
...
- Closing a plan includes a linked-condition review. Resolve it, retain it with an updated severity and follow-up, and optionally create a reminder. Completed plans can be duplicated or amended with a required reason.
- Review plans and application history from each specimen. Treatment events also appear on the Plant Health Timeline.
- Use Treatment reports for usage, active work, adverse reactions, conservative outcome summaries, and CSV export with explicit units. Small samples are labeled and percentages are withheld until the sample is large enough.
...
- Treatment definitions used by active plans require explicit confirmation before archival. Existing plans and historical snapshots remain intact.
- Treatment library, plan, application, report, and search data is available only to authenticated collection members; public collection pages do not expose internal treatment records.
