...
- Gardeners can move plants between existing active locations one at a time or use batch move preview for direct-only or direct-plus-nested moves.
...
- Moving a plant into a quarantine-type location prompts but does not automatically create a quarantine record.
